...
n_epochs = 200
weights = Variable(torch.Tensor((0.9,5.0)).float().cuda()).data
print(weights)
model = FCN8(2).cuda()
#model = SegNet(2).cuda()
...
for epoch in range(n_epochs):
    ...
    v = model.va(data[2],data[3],batch_size,n_valid_batch,model)                            
    l = model.te(data[4],data[5],batch_size,n_test_batch,model)
